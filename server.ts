...
// Global state in-memory database of registered sarees
let registeredSarees = [
  {
    id: "AT-2026-948F",
    weaverName: "Lakshmi Devi",
    weaverAge: 52,
    weaverBio: "A third-generation master weaver from Kuthampully, Kerala, specializing in traditional Kasavu cotton weave with pure silver and gold borders. She has been weaving since she was 14 years old.",
    village: "Kuthampully",
    ...
    patternStyle: "kasavu",
    mainColor: "#faf6f0", // Warm creamy off-white
    accentColor: "#d4af37", // Golden zari
    seed: 4891,
  },
  {
    id: "AT-2026-302K",
    ...
    weaverAge: 61,
    weaverBio: "Awarded the National Handloom Merit Certificate in 2018. Specialist in pure double-warp Kanchipuram silk with complex structural jacquard-style border patterns.",
    village: "Kanchipuram",
  ...
  },
  {
    ...
    weaverAge: 39,
    weaverBio: "Highly recognized for reviving traditional Peshwai borders and intricate peacock pallus. She operates a workshop supporting 15 local women weavers.",
    village: "Paithan",
    ...
    mainColor: "#026c7d", // Peacock Teal
    accentColor: "#e65c00", // Accent orange
    seed: 3345,
  }
];
...
app.post("/api/register", (req, res) => {
  ...
  registeredSarees.unshift(newSaree); // Add to the top
...
app.post("/api/verify", async (req, res) => {
  ...
  if (!referencePhoto || !shopperPhoto) {
    return res.status(400).json({ error: "Reference fingerprint photo and fresh shop scan photo are required." });
  }
  ...
  // If no Gemini client is initialized, use the smart simulation engine
  if (!ai) {
    ...
    if (scanType === "matching") {
      return res.json({
        ...
        detailedAnalysis: {
          weaveStructure: "A high-precision horizontal and vertical spacing cross-correlation of 98.4% indicates that both snapshots contain identical thread densities and crossing coordinates.",
          threadTension: "Identical micro-laxity and human-tension patterns are visible. The natural hand-weaving weave signatures match perfectly.",
          patternAlignment: "The border pattern intersections and horizontal transitions line up perfectly with a minor 1.2-degree angle correction applied."
        },
      ...
      return res.json({
        ...
        detailedAnalysis: {
          weaveStructure: "The scanned saree has a mathematically perfect weave. Warp and weft threads maintain an absolute parallel grid, which is physically impossible on a hand-operated traditional loom.",
          threadTension: "Tension variance index is 0.0%, displaying absolute uniform machine tension. The registered saree has an organic 4.7% human tension signature which this scan lacks.",
          patternAlignment: "A perfect geometric repetition signature is present, confirming this is a machine-made copy (powerloom fake) exploiting a duplicated sticker."
        },
      ...
      return res.json({
        ...
        detailedAnalysis: {
          weaveStructure: "Color channels and thread count ratios are entirely different. There is no weave intersection match.",
          threadTension: "No correlation found between the registered human tension profiles and this scanned item.",
          patternAlignment: "Completely unrelated visual pattern. This is either a mismatched item or a counterfeit tag."
        },
  ...
  // Real Gemini Multi-Modal Comparison
  try {
...
You MUST respond strictly in the following JSON format. Do not write any markdown (like \`\`\`json or \`\`\`), do not include any text before or after the JSON.
Response Schema:
{
  "isMatch": boolean,
  "matchScore": number,
  ...
  "recommendation": "A friendly authenticating statement or warning, mentioning the weaver name (${weaverName || "registered weaver"})."
    ...
    res.status(500).json({ error: "AI matching failed. Please check backend logs or try again.", details: err.message });
  }
});

// Configure Vite integration or production serving
